...
def lstm_cell_forward(Xt, h_prev, c_prev, parameters):
    ...
    concat[:, :H] = h_prev # self.h0 = np.zeros((self.N, self.H))
    concat[:, H:] = Xt
...
def lstm_cell_backward(dh_next, dc_next, cache):
    ...
    dh_prev = np.dot(dft, parameters['Wf'][:H, :].T) + np.dot(dit, parameters['Wi'][:H, :].T) + np.dot(dcct, parameters['Wc'][:H, :].T) + np.dot(dot, parameters['Wo'][:H, :].T)
    dc_prev = dc_next * ft + ot * (1 - np.square(np.tanh(c_next))) * ft * dh_next
    dXt = np.dot(dft, parameters['Wf'][H:, :].T) + np.dot(dit, parameters['Wi'][H:, :].T) + np.dot(dcct, parameters['Wc'][H:, :].T) + np.dot(dot, parameters['Wo'][H:, :].T)
