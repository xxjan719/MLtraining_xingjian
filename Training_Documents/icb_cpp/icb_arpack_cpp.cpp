...
template <typename Real>
void my_matrix_vector_product(const Real* x, Real* y) {
  // TODO: Multiply the vector 'x' by your matrix 'A' and store the result in 'y'.
  // This will depend on the specific structure of your matrix.
}

template <typename Real>
void real_symmetric_runner(double const& tol_check, arpack::which const& ritz_option) {
  const a_int N      = 1000;   // dimension of the problem TODO: USER ADJUSTABLE
  const a_int nev    = 9;      // number of eigenvalues requested TODO: USER ADJUSTABLE
  const a_int ncv    = 2 * nev + 1;
  const a_int ldv    = N;
  const a_int ldz    = N;
  const a_int lworkl = ncv * (ncv + 8);
  const a_int rvec   = 1;      // need eigenvectors TODO: USER ADJUSTABLE
  ...
  if (info < 0 || iparam[4] < nev) { /*arpack may succeed to compute more EV than expected*/
    ...
    std::cout << "ERROR in saupd: iparam[4] " << iparam[4] << ", nev " << nev << ", info " << info << std::endl;
